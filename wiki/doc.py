...
def doc_add(name):
    log('doc_add', name)
    o = WikiDoc.objects.get_or_create(name=name)[0]
    #o.abc = abc
    o.save()
...
# Delete
def doc_delete(pk):
    log('WikiDoc delete', pk)
    WikiDoc.objects.filter(pk=pk).delete()
...
# Get
def doc_lookup(pk):
    return WikiDoc.objects.get(pk=pk)


# Get
def doc_get(client, name):
    return WikiDoc.objects.get(client=client, name=name)


# HTML
def doc_html(title):
    path = join('WikiDocuments', title)
    if doc_exists(path):
...
# Path
def doc_path(title):
    return join('WikiDocuments', title)
...
def doc_query(select=None):
    if select:
        objects = WikiDoc.objects.filter(client=select)
    else:
        objects = WikiDoc.objects.all()
...
#     return WikiDoc.labels(), doc_query(select)
